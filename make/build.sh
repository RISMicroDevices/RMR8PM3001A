...
build_proj() {
    ...
    # MODIFIED: get all external vsrc include subfolders
    VSRC_EXT_SUB_FOLDER=`find $VERILATOR_EXT_INCLUDE -type d`
    for SUBFOLDER in ${VSRC_EXT_SUB_FOLDER[@]}
    do
        INCLUDE_EXT_VSRC_FOLDERS="$INCLUDE_EXT_VSRC_FOLDERS -I$SUBFOLDER"
    done

    #MODIFIED: get all mixed elaboration cpp files
    if [[ -n "$VERILATOR_MELA_PATH" ]]; then
...
while getopts 'he:bt:sa:f:l:gwcdm:r:vi:p:' OPT; do
    case $OPT in
        ...
        v) VERILATOR_PARAM="$OPTARG";;
        i) VERILATOR_EXT_INCLUDE="$OPTARG";;
        p) VERILATOR_MELA_PATH="$OPTARG";;
        ?) help;;
    esac
done
