...
#include "Vsim_cross_buffer4_priority.h"
static Vsim_cross_buffer4_priority* dut_ptr;
#else
#include "Vsim_cross_buffer4.h"
static Vsim_cross_buffer4* dut_ptr;
#endif
...
void testbench_0(int& t)
{
    ...
    dut_ptr->prev2_i_data  = 0;
    dut_ptr->prev2_i_valid = 0;

    dut_ptr->prev3_i_data  = 0;
    dut_ptr->prev3_i_valid = 0;
    ...
    while (1)
    {
        ...
        if (dut_ptr->next_i_ready && dut_ptr->next_o_valid)
        {
            ...
            else if (PRT(dut_ptr->next_o_data) == 2)
            {
                if (!complete_p2)
                {
                    records_p2.push_back(dut_ptr->next_o_data);

                    complete_p2 = gen_t_p2 == 0x100;
                }
                else
                    printf("[#0] Dataflow corruption detected. Received new data from PORT 2 after dataflow finished.\n");
            }
            else if (PRT(dut_ptr->next_o_data) == 3)
            {
                if (!complete_p3)
                {
                    records_p3.push_back(dut_ptr->next_o_data);

                    complete_p3 = gen_t_p3 == 0x100;
                }
                else
                    printf("[#0] Dataflow corruption detected. Received new data from PORT 3 after dataflow finished.\n");
            }
            else
        ...
        else if (prev2_nready == 0)
        {
            dut_ptr->prev2_i_data  = GEN(2, gen_t_p2);
            dut_ptr->prev2_i_valid = 1;

            if (dut_ptr->prev2_o_ready)
            {
                prev2_nready = rand() % 8;
                gen_t_p2++;
            }
        ...
        {
            dut_ptr->prev2_i_data  = 0xDEADBEEF;
            dut_ptr->prev2_i_valid = 0;
        ...
        else if (prev3_nready == 0)
        {
            dut_ptr->prev3_i_data  = GEN(3, gen_t_p3);
            dut_ptr->prev3_i_valid = 1;

            if (dut_ptr->prev3_o_ready)
            {
                prev3_nready = rand() % 8;
                gen_t_p3++;
            }
        ...
        {
            dut_ptr->prev3_i_data  = 0xDEADBEEF;
            dut_ptr->prev3_i_valid = 0;
        ...
        if (complete_p0 && complete_p1 && complete_p2 && complete_p3)
            break;
    }
    ...
    if (records_p2.size() == 0x100)
        printf("[#0] Record sequence from PORT 2 size matched.\n");
    else
    {
        printf("[#0] Record sequence from PORT 2 size unmatched. Result: %ld, Ref: %d.\n", records_p2.size(), 0x100);
        passed = 0;
    }

    if (records_p3.size() == 0x100)
        printf("[#0] Record sequence from PORT 3 size matched.\n");
    else
    {
        printf("[#0] Record sequence from PORT 3 size unmatched. Result: %ld, Ref: %d.\n", records_p3.size(), 0x100);
        passed = 0;
    }
    ...
    //
    int pattern_unmatch_p2 = 0;
    int cti_p2 = 0;

    list<int>::iterator iter_p2 = records_p2.begin();

    while (iter_p2 != records_p2.end())
    {
        if (*iter_p2 != GEN(2, cti_p2))
        {
            printf("[#0] Pattern of PORT 2 unmatched at %3d. Result: %x, Ref: %x\n", cti_p2, *iter_p2, GEN(2, cti_p2));
    ...
    if (!pattern_unmatch_p2)
        printf("[#0] Pattern test of PORT 2 Passed.\n");
    else
    {
        printf("[#0] Pattern test of PORT 2 Failed. %d unmatched record(s) found.\n", pattern_unmatch_p2);
        passed = 0;
    }

    //
    int pattern_unmatch_p3 = 0;
    int cti_p3 = 0;

    list<int>::iterator iter_p3 = records_p3.begin();

    while (iter_p3 != records_p3.end())
    {
        if (*iter_p3 != GEN(3, cti_p3))
        {
            printf("[#0] Pattern of PORT 3 unmatched at %3d. Result: %x, Ref: %x\n", cti_p3, *iter_p3, GEN(3, cti_p3));
    ...
    if (!pattern_unmatch_p3)
        printf("[#0] Pattern test of PORT 3 Passed.\n");
    else
    {
        printf("[#0] Pattern test of PORT 3 Failed. %d unmatched record(s) found.\n", pattern_unmatch_p3);
        passed = 0;
    }
...
#ifdef CROSS_BUFFER_PRIORITY
    printf("[##] Testing on module \'common_cross_buffer4_priority\'.\n");
#else
    printf("[##] Testing on module \'common_cross_buffer4\'.\n");
#endif
...
#ifdef CROSS_BUFFER_PRIORITY
    dut_ptr = new Vsim_cross_buffer4_priority;
    printf("[##] Priority 4-to-1 Cross Buffer module 'common_cross_buffer4_priority' selected.\n");
#else
    dut_ptr = new Vsim_cross_buffer4;
    printf("[##] Pseudo-LRU 4-to-1 Cross Buffer module 'common_cross_buffer4' selected.\n");
#endif
